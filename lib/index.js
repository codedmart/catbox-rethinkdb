...
internals.Connection.prototype.flush = function() {
    ...
    setInterval(function() {
        ...
        try {
            ...
            return this.table.between(RethinkDB.minval, RethinkDB.now(), {index: 'expiresAt'}).delete().run();
        }
...
internals.Connection.prototype.createDb = function () {
    ...
    return RethinkDB.dbList().run()
    .then(function(dbs) {

        if (!Hoek.contain(dbs, self.settings.db)) {

            return RethinkDB.dbCreate(self.settings.db).run();
        }
    });
};
